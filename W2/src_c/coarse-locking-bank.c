...
// Coarse Locking - 1 mutex cho toàn bộ critical section
...
void * transactions(void * args){
    ...
    for(i = 0; i < NUM_TRANS; i++){
        ...
        // CRITICAL SECTION - Coarse Locking toàn bộ
...
int main(int argc, char * argv[]){
    ...
    // Khởi tạo 1 mutex lock
    pthread_mutex_init(&mutex, NULL);
    
    // allocate array of thread identifiers
    threads = calloc(n_threads, sizeof(pthread_t));
    
    printf("=== BANK SIMULATION WITH COARSE LOCKING ===\n");
    printf("Initial Balance: %d\n", INIT_BALANCE);
    printf("Number of threads: %d\n", n_threads);
    printf("Transactions per thread: %d\n", NUM_TRANS);
    printf("Total transactions: %d\n", n_threads * NUM_TRANS);
    printf("Locking strategy: Coarse Locking (1 mutex for all variables)\n\n");
    ...
    if(difference == 0) {
        printf("✅ CONSISTENT - Perfect thread synchronization with coarse locking!\n");
    } else {
        printf("❌ INCONSISTENT - Coarse locking failed unexpectedly!\n");
    }
